...
public class ZebraGameController : MonoBehaviour
{
    ...
    // Tracks permanent (royal delegation) policies enacted at each location, by effect type and count.
    ...
    //   mCardPlayEnabled = true only in Turn Phase 1, when the hand may be played.
    //   mBuyingEnabled   = true only in Turn Phase 2, when new cards may be bought.
    //   mIntegrated      = a TurnController exists, so the map turn controls the card flow.
    private bool mCardPhaseActive = false;
    private bool mCardPlayEnabled = false;
    private bool mBuyingEnabled = false;
    private bool mIntegrated = false;

    [Header("Card definitions (ScriptableObjects)")]
    ...
    private void Start()
    {
        ...
        if (mIntegrated)
        {
            ...
            // the player can see it while handling the first event.
            mStartRoundButton.gameObject.SetActive(false);
            mEndRoundButton.gameObject.SetActive(false);   // the Turn Phase Button controls rounds
            StartTurnHand();
        }
        else
        {
    ...
    private void ResolveBaseReferences()
    {
        ...
        if (mStats == null)
            Debug.LogWarning("ZebraGameController: no StatManager in scene — card stat effects will be skipped.");
        if (mTurns == null)
            Debug.LogWarning("ZebraGameController: no TurnController in scene — minister limits will be skipped.");
    }

    // ==== Map-turn integration: the TurnPhaseButton / EventManager drive these entry points. ====

    // Turn start (game start, and right after the previous turn ends): draw this turn's hand
    // and show the board so the player can see their cards while deciding the event (Phase 0).
    // Playing is NOT enabled here — that happens in Phase 1 via EnableCardPlay().
    public void StartTurnHand()
    {
        if (mCardPhaseActive) return;
        mCardPhaseActive = true;
        mCardPlayEnabled = false;
        mBuyingEnabled = false;
        if (mTurns != null) mRoundNumber = mTurns.GetTurnCount();
    ...
    {
        mCardPlayEnabled = true;
        SetStatus("Hover over a card. Click once to hold it, then click again to play.",
                  "将鼠标移到卡牌上。点击一次拿起，再次点击打出。");
        RefreshInterface();
    }

    // Turn Phase 1 -> 2: reveal the "reveal" effects of the cards left unplayed in hand,
    // discard them, then switch the board into buying mode for Phase 2.
    public void RevealLeftoverCards()
    {
        if (!mCardPhaseActive) return;
        StopAllCoroutines();
        ClearSelection();

        foreach (CardModel card in new List<CardModel>(mHand))
        {
            ApplyRetainEffect(card);
            if (mHandViews.TryGetValue(card, out CardView view) && view != null)
            {
        ...
        mCardPlayEnabled = false;           // Phase 1 play is over
        mBuyingEnabled = true;              // Phase 2: buying new cards is now allowed
        mPhase = GamePhase.PlayerAction;
        SetStatus("Reveal effects resolved. Buy new cards, then continue.", "揭示效果已结算。可购买新卡，然后继续。");
        RefreshInterface();
    }

    // Turn Phase 2 -> 3: finished with cards this turn; clear and hide the board.
    public void EndCardTurn()
    {
        mCardPhaseActive = false;
        mCardPlayEnabled = false;
        mBuyingEnabled = false;
        StopAllCoroutines();
        ClearSelection();

        // Clear any leftover highlights (occupancy is reset by TurnController.EndTurn).
        if (mLocations != null)
            foreach (ClickOnLocation location in mLocations)
                location.SetHighlighted(false);

        mPhase = GamePhase.WaitingToStart;
        ShowCardInterface(false);
    }

    private void ShowCardInterface(bool visible)
    {
        if (mCanvas != null) mCanvas.gameObject.SetActive(visible);
    }
    ...
    {
        // Build the starting deck from the assigned CardSO assets.
        if (startingDeck != null)
        {
            foreach (CardSO so in startingDeck)
                if (so != null) mOwnedCards.Add(CreateCard(so));
        }
        mDrawPile.AddRange(mOwnedCards);
        Shuffle(mDrawPile);

        // Cards available to buy in Phase 2.
        if (marketCards != null)
        {
            foreach (CardSO so in marketCards)
                if (so != null) mMarketCards.Add(CreateCard(so));
        }
    }

    // Copy a CardSO definition into a fresh runtime CardModel instance.
    ...
    // visit and all future matching location visits during this game.
    ...
    private IEnumerator StartRoundRoutine()
    {
        ...
        // Ministers are a per-map-turn resource owned by TurnController; they are NOT
        // refilled per card round. Location occupancy is reset by TurnController.EndTurn
        // (which calls ResetObject on each ClickOnLocation); here we just clear highlights.
        if (mLocations != null)
            foreach (ClickOnLocation location in mLocations)
                location.SetHighlighted(false);
        ...
        mPhase = GamePhase.PlayerAction;
        if (mIntegrated && !mCardPlayEnabled)
            SetStatus("Your hand for this turn. Handle the event, then play cards in Phase 1.",
                      "本回合手牌。先处理事件，然后在阶段 1 出牌。");
        else
            SetStatus("Hover over a card. Click once to hold it, then click again to play.",
                      "将鼠标移到卡牌上。点击一次拿起，再次点击打出。");
        RefreshInterface();
    ...
    private void BeginPlaySelectedCard()
    {
        ...
        if (mTurns != null && !mTurns.HasMinisterAvailable())
        {
    ...
    // effect (from LocationEffects) and consumes a minister; this only handles the card.
    private IEnumerator ResolvePlayedCardRoutine()
    {
        mPhase = GamePhase.Animating;
        ...
        if (mLocations != null)
            foreach (ClickOnLocation item in mLocations)
                item.SetHighlighted(false);
        LayoutHand();

        PrepareCardForCanvasAnimation(view);
        // Cards carrying a permanent policy are consumed (removed from the deck) instead of
        // going to the discard pile, since their effect is now stationed at the location.
    ...
    private void ApplyRetainEffect(CardModel card)
    {
        ...
        // Temporary reveal-phase resources come only from retained cards (reset each turn end).
        if (mStats != null)
        {
            if (card.MajestyGain != 0) mStats.UpdateMajesty(card.MajestyGain);
            if (card.FightGain != 0) mStats.UpdateFight(card.FightGain);
        }

        if (card.RetainEffect == RetainEffectType.PublicOpinionUp)
        {
            if (mStats != null) mStats.UpdateResource(1, 0, 0);   // PublicOpinion +1
    ...
    private void ConfirmOverlayAction()
    {
        ...
        {
            if (mStats == null || mStats.GetMajesty() < deleteMajestyCost)
            {
                SetStatus("Not enough Majesty to delete a card.", "威望不足，无法删除卡牌。");
                return;
            }
            mStats.UpdateMajesty(-deleteMajestyCost);
        ...
        {
            int cost = mOverlaySelectedCard.MajestyCost;
            if (mStats == null || mStats.GetMajesty() < cost)
            {
                SetStatus("Not enough Majesty to buy this card.", "威望不足，无法购买此卡。");
                return;
            }
    ...
    private void RefreshInterface()
    {
        if (mStats != null)
        {
            mStatsText.text = mUseChinese
                ? "金币 " + mStats.GetGold() + "    民意 " + mStats.GetPO() + "    军力 " + mStats.GetMS() + "    权威 " + mStats.GetAL()
                : "Gold " + mStats.GetGold() + "    PO " + mStats.GetPO() + "    MS " + mStats.GetMS() + "    AL " + mStats.GetAL();
        }

        int ministersLeft = mTurns != null ? mTurns.GetMinistersLeft() : 0;
        int maxMinisters = mTurns != null ? mTurns.GetMaxMinisters() : 0;
    ...
    private Button CreateButton(string name, Transform parent, string label, Vector2 anchor, Vector2 position, Vector2 size, Color color)
    {
        ...
        GameUITheme.StyleButton(button);   // burgundy lacquer strip texture (9-sliced)
        return button;
    }
    ...
    private Button CreatePileButton(string name, Transform parent, string label, Vector2 position, out Text countText, out Text nameText)
    {
        ...
        GameUITheme.StyleCardBack(button.GetComponent<Image>());   // BlackGold card-back texture for the pile
